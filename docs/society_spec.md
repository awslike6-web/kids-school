...
- **서가 목록 (`SOCIETY_STORYBOOK_LIBRARY`)**:
  ...
  - **[5학년 2학기 1편]**: ⏳ `1단원 : 옛사람들의 삶과 문화 (선사~고조선)` (`history_time_travel_storybook.html`) - 박물관 유물함, 주먹도끼, 빗살무늬 토기, 고인돌, 8조법.
...
- **음성 싱크 표준**:
  - 앞표지부터 마지막 장까지 실제 책 대사 텍스트와 `edge-tts` (`ko-KR-SunHiNeural`) 음성이 1:1로 완전 일치.
